...
// f1+f2: named layout rects — single source of truth for both hit-testing and drawing
...
def gameLoop(coin: Ptr[Texture2D])(using Zone): Unit =
    ...
    while !Raylib.WindowShouldClose() do
        ...
        state = handleClick(mouse)(state.tick(dt))
        Raylib.BeginDrawing()
...
private def handleClick(mouse: Ptr[Vector2])(s: GameState): GameState =
    ...
    else if Raylib.SN_CheckCollisionPointRec(mouse, clickButtonRect) then
        s.startAnimation()
    else if Raylib.SN_CheckCollisionPointRec(mouse, clickUpgradeRect) then
        s.tryBuyClickUpgrade
    else if Raylib.SN_CheckCollisionPointRec(mouse, passiveUpgradeRect) then
        s.tryBuyPassiveUpgrade
    else s
...
private def drawScene(s: GameState, coin: Ptr[Texture2D])(using Zone): Unit =
    ...
    drawCenteredText(
      toCString(s"Currency: ${s.currency}"),
      0,
      WINDOW_W,
      CURRENCY_Y,
      FONT_LARGE,
...
private def drawClickArea(
    ...
    drawCenteredText(
      toCString(s"(+$clickPower)"),
      r.x.toInt,
      r.width.toInt,
      CLICK_LABEL_Y + FONT_TITLE,
      FONT_LARGE,
      BLACK
    )
...
private def drawUpgrades(s: GameState)(using Zone): Unit =
    drawUpgradeButton(
      clickUpgradeRect,
      c"Click Power",
      toCString(s"Level: ${s.clickPower}"),
      c"+1 per click",
      toCString(s"Cost: ${s.clickCost}"),
      s.currency >= s.clickCost
    )
    drawUpgradeButton(
      passiveUpgradeRect,
      c"Passive Income",
      toCString(s"Level: ${s.passiveRate}"),
      c"+1 per second",
      toCString(s"Cost: ${s.passiveCost}"),
      s.currency >= s.passiveCost
    )
